def nexafs_S_edge(t=1):
    ...
    for name, xs, ys in zip(names, x, y):
        ...
        for wa in waxs_arc:
            ...
            yield from bps.mv(energy, 2470)  # 💡 smi_plans: this gentle step-down to a parking energy (2470→2450) is no longer needed — energy_axis/move_energy_fb do this for you: a plain bps.mv(energy, E) -- the energy device itself manages the DCM feedback, the undulator gap and the harmonic, so no manual feedback handling, energy hops, or beam re-seek is needed (pass flux_signal=/flux_threshold= if you want a beam-loss guard). (Not broken, just no longer needed.)
...
def saxs_S_edge_Hoang_2022_2(t=0.5):
    ...
    for name, xs, ys in zip(names, x, y):
        ...
        for i, wa in enumerate(waxs_arc):
            ...
            yield from bps.mv(energy, 2480)  # 💡 smi_plans: this gentle step-down to a parking energy (2480→2450) is no longer needed — energy_axis/move_energy_fb do this for you: a plain bps.mv(energy, E) -- the energy device itself manages the DCM feedback, the undulator gap and the harmonic, so no manual feedback handling, energy hops, or beam re-seek is needed (pass flux_signal=/flux_threshold= if you want a beam-loss guard). (Not broken, just no longer needed.)
            yield from bps.mv(energy, 2450)
...
def tensile_single_Hoang_2022_2(t0, t=0.5):
    ...
    for rot in rotations:
        ...
        for wa in waxs_arc:
            ...
            yield from bps.sleep(2)  # 💡 smi_plans: this settle wait after the energy move is no longer needed — move_energy_fb/energy_axis do this for you: a plain bps.mv(energy, E) -- the energy device itself manages the DCM feedback, the undulator gap and the harmonic, so no manual feedback handling, energy hops, or beam re-seek is needed (pass flux_signal=/flux_threshold= if you want a beam-loss guard). (Not broken, just no longer needed.)

            # Move energy slowly
            if e == 2500:
                yield from bps.mv(energy, 2490)  # 💡 smi_plans: this gentle step-down (2490→2480→2470) is no longer needed — energy_axis/move_energy_fb do this for you: a plain bps.mv(energy, E) -- the energy device itself manages the DCM feedback, the undulator gap and the harmonic, so no manual feedback handling, energy hops, or beam re-seek is needed (pass flux_signal=/flux_threshold= if you want a beam-loss guard). (Not broken, just no longer needed.)
