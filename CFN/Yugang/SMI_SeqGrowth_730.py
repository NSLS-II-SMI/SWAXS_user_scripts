...
GEOMETRY (2026C2)
-----------------
Three reactors in a horizontal row. **MDrive3 (``MDrive.m3``) is the only motor that
moves**; the Huber stage (X -28, Y -30, Z 2) is set by hand and left alone::

    R0 = 50 mm   (48-53)      R1 = 85 mm   (82-88)      R2 = 120 mm  (117-123)

BEAM-DAMAGE TWEAK (long runs)
-----------------------------
Re-exposing one spot for hours cooks it. In a long run each sweep steps every reactor to
the NEXT spot in a +/-2 mm window at 0.2 mm — 20 spots — and wraps around at the end::

    R0@p00 -> R1@p00 -> R2@p00 -> R0@p01 -> R1@p01 -> R2@p01 -> ... -> p19 -> p00 -> ...

The spot index is in the file name (``_p07_``) and so is the true motor read-back
(``_x051.40_``). ``tweak=False`` pins every frame to the reactor centre.
...
name carries ONE ``_T<value>_`` token::
    ...
    DR_AuC8C12_b01_R0_T40.0_p04_...        <- after TempX(40)
...
#   SMI : '/nsls2/data/smi/legacy/results/data/2024_3/313765_Zhang/Dropbox_Com/'
...
# is +/-2 mm: it stays inside the reactor with a margin at both ends.
POS_DICT = {'R0': 50.0, 'R1': 85.0, 'R2': 120.0}
REACTORS = ['R0', 'R1', 'R2']            # default order; DR.set_reactors(1|2|3|[...]) narrows it

# ---- beam-damage tweak --------------------------------------------------------------
# +/-2.0 mm at 0.2 mm = 20 spots per reactor. 0.2 mm is about the beam width, so
# consecutive spots do not overlap; 20 sweeps is ~1 h at 3 min/sweep before a spot is
# revisited. Change with DR.set_tweak(radius, step).
TWEAK_RADIUS_MM = 1.2
TWEAK_STEP_MM = 0.2
...
def temp_token(temp_c):
    """'T40.0' for 40, '' when the temperature is unknown.
...
def strip_temp_token(text):
    """Remove any ``T<number>`` token from a name.
    ...
    kept = []
    for part in parts:
        if len(part) > 1 and part[0] == 'T':
            try:
                float(part[1:])
                continue                          # it is a temperature token -- drop it
            except ValueError:
                pass
        kept.append(part)
...
# --------------------------------------------------------------------------- #
#   X (motorX): MDrive.m3  -- reactor-to-reactor AND the beam-damage tweak. DRIVEN.
#   Y (motorY): stage.y    -- droplet height. READ ONLY (it goes into the file name);
#                             set it by hand once, with the Huber stage.
...
try:
    motorY = stage.y        # noqa: F821  read-only height, for the file name
except NameError:
    motorY = None


def set_motors(x=None, y=None):
    """Bind the driven horizontal axis (x) and the read-only height (y).
    ...
    if y is not None:
        motorY = y
    print('SMI_SeqGrowth: motorX=%s (driven)  motorY=%s (read only)' %
...
def tweak_offsets(radius=None, step=None):
    """The beam-damage spot list: offsets (mm) from a reactor centre.

    Default +/-2.0 mm at 0.2 mm = **20 spots**, from -2.0 to +1.8. The +2.0 end is left
    out on purpose so the list is exactly 20 long and wrapping from the last spot to the
    first is one full step, not a double exposure of the same place.
    """
...
class DropletReactor(object):
    ...
    def set_tweak(self, radius=None, step=None, on=True):
        """Resize the beam-damage window. ``DR.set_tweak(on=False)`` pins the centre."""
        self.offsets = tweak_offsets(radius, step)
        self.tweak = bool(on)
        self.pos_index = 0
        print('SMI_SeqGrowth: tweak=%s  %d spot(s)  %.2f .. %.2f mm  step %.2f mm' %
              (self.tweak, len(self.offsets), self.offsets[0], self.offsets[-1],
               (self.offsets[1] - self.offsets[0]) if len(self.offsets) > 1 else 0.0))
        return self.offsets
    ...
    def set_temp(self, temp_c, why='set by hand', quiet=False):
        ...
        Call it by hand for a manual run at a known temperature. It changes NOTHING on
        the block -- the beamline drives no heater; it only records what the lab said.
        """
        old = self.temp_c
        if temp_c is None:
            self.temp_c = None
            ...
            try:
                ...
                return self.temp_c
        changed = (old is None) != (self.temp_c is None) or (
            old is not None and self.temp_c is not None and abs(old - self.temp_c) > 1e-9)
    ...
    def temp_tag(self):
        """The ``T40.0`` token for the current temperature ('' when it is unknown)."""
        return temp_token(self.temp_c)
    ...
    def _consume_temp(self, comm_dir=None, announce=True):
        ...
        if temp_c is None:
            ...
            return None
        old = self.temp_c
        self.set_temp(temp_c, why=TEMPX_NAME)
        if announce and (old is None or abs(old - self.temp_c) > 1e-9):
            mail('SMI: T -> %.1f C (%s)' % (self.temp_c, self.sample_pref),
                 'The block reached %.1f C (was %s).\nFrames are now named %s.\n'
                 'Reactors: %s   spot p%02d.'
                 % (self.temp_c, old, self.temp_tag(), self.reactors,
                    self.pos_index % max(1, len(self.offsets or [0]))),
                 key='sg_temp', min_interval_s=0.0)
        return self.temp_c

    def _offset(self):
        """The offset (mm) the current sweep uses, and its index for the file name."""
        if not self.tweak or not self.offsets:
            return 0.0, 0
        index = int(self.pos_index) % len(self.offsets)
        return float(self.offsets[index]), index

    def goto_Pos(self, pos_des, offset=0.0):
        """Move MDrive3 X to a reactor (+ tweak offset). Nothing else is driven -- the
        three reactors share one height, set by hand on the Huber stage."""
        ...
        RE(bps.mv(motorX, x))       # noqa: F821  (RE/bps from the bluesky profile)
    ...
    def setup_beamline(self, waxs_angle=None, take_camera=False):
        ...
        this DOES do is the two things that silently ruin a night if they are skipped:

          * move the real WAXS motor (``move_waxs()`` in YZhang_SMI_Base.py) so the angle
            in every file name is the angle the detector is actually at, and
        ...
        print('SMI_SeqGrowth: motorX=%s (x=%s)  motorY=%s (y=%s)  waxs=%.2f  det=%s m'
    ...
    def check(self, comm_dir=None, collect=False, exp_time=1, name='check'):
        ...
        print('  motorX     : %-10s x=%s   (DRIVEN)' %
              (getattr(motorX, 'name', motorX), _num(_axis_position(motorX))))
        print('  motorY     : %-10s y=%s   (read only)' %
              (getattr(motorY, 'name', motorY), _num(_axis_position(motorY))))
        print('  waxs       : %.2f deg     det: %s m' %
              (_waxs_angle(), _num(_saxs_distance(), 5, 2)))
        print('  reactors   : %s' % (self.reactors,))
        print('  positions  : %s' % (dict((r, self.pos_dict[r]) for r in self.reactors),))
        print('  tweak      : %s   %d spot(s)  %+.2f .. %+.2f mm  (next p%02d)' %
              (self.tweak, len(offsets), offsets[0], offsets[-1],
               self.pos_index % len(offsets)))
        ...
        return {'reactors': list(self.reactors),
                ...
                'tweak': self.tweak, 'n_spots': len(offsets),
        ...
        try:
            while done is None:
                ...
                for pos in reactors:
                    ...
                    # appending to it, and one frame can only ever carry one T.
    ...
    def collect_once(self, name, exp_time=1, rxn_pos=None, tweak=False, sleep_time=0.0,
        ...
        """
        if temp_c is not False:
            self.set_temp(temp_c, why='collect_once()')
    ...
    def run_manual(self, name, exp_time=1, run_time=60 * 60, rxn_pos=None, tweak=True,
                   ...
                   waxs_angle=None, setup=True, verbosity=3):
        ...
        """
        if temp_c is not False:
            self.set_temp(temp_c, why='run_manual()')
        reactors = self._as_reactors(rxn_pos) if rxn_pos is not None else list(self.reactors)
        if setup:
            self.setup_beamline(waxs_angle=waxs_angle, take_camera=take_camera)
        base = _safe_name(name)
        ...
        print('  tweak      : %s (%d spot(s), next p%02d)' %
              (tweak, len(self.offsets), self.pos_index % max(1, len(self.offsets))))
    ...
    def run_auto(self, rxn_pos=None, sleep_time=12,
        ...
        One pass of the loop:
          ...
          2. cycle the selected reactors, measuring ``exp_time`` at each (MDrive3 X only),
             stepping the beam-damage spot after each full sweep, naming every frame
             after THAT filename and the CURRENT temperature;
        ...
        while time.time() < t_end:
            ...
            mail('SMI: window OPEN -- %s' % filename,
                 ...
                 % (base_name, payload.get('batch'), payload.get('n_batches'),
                    ...
                    self.pos_index % max(1, len(self.offsets or [0])), cap_min),
